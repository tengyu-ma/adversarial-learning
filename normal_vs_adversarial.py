...
class NormalVsAdversarial:
    ...
    def adversarial_test_denoised(self, epsilon=0.1, ds_name='threshold_method', para=0.5):
        test_size = 20000
        data, sess, x, y_, keep_prob = self.data, self.sess, self.x, self.y_, self.keep_prob
        ...
        if ds_name == 'threshold_method':
            x_test_denoised = ds.threshold_method(x_test_adversarial, para)
        else:
            x_test_denoised = ds.threshold_method(x_test_adversarial, para)
    ...
    def adversarialize(self, ad_name, x_test_normal, y_test_normal, epsilon=0.1):
        """
    ...
    def save_NvA_images(self, data_set_name, image_num):
        ...
        x_adv, y_adv, noise = self.adversarialize('random_method', x, y_, 0.25)  # adversarial case
        denoised_tmp = ds.threshold_method(x, 0.5)
        selected = list(random.sample(util.TEST_SIZE, image_num))
        for i in selected:
