...
class AutoencoderEvaluator(Evaluator):
    ...
    def _error(self,
        ...
        with tf.Graph().as_default():
            ...
            # if input_type ==  InputType.test:
            #     log_io(images, predictions)
            ...
            with tf.Session(config=tf.ConfigProto(
                ...
                try:
                    ...
                    while step < num_iter and not coord.should_stop():
                        error_value, pred, img, lbs = sess.run([loss, predictions, images, labels])
                        output_images.append(pred)
                        input_images.append(img)
                        input_labels.append(lbs)
                        step += 1
                        error_sum += error_value

                    output_images = np.reshape(output_images, (-1, 24,24,3))
